...
public class BIFReader extends BayesNet implements TechnicalInformationHandler {
  ...
  /**
   * Count the number of correct arcs that are present in both networks with the same orientation.
   * 
   * @param other network to compare with
   * @return number of correct arcs
   */
  public int correctArcs(BayesNet other) {
    try {
      Sync(other);
      int nCorrect = 0;
      for (int iAttribute = 0; iAttribute < m_Instances.numAttributes(); iAttribute++)
      {
        ...
        {
          int parentIndex = m_ParentSets[iAttribute].getParent(iParent);
          if (other.getParentSet(m_order[iAttribute]).contains(m_order[parentIndex]))
          {
            nCorrect++;
          }
        }
  ...
  public int correctArcs(int [][] adjMatrix, BayesNet other) {
    try {
      Sync(other);
      ...
      for (int iAttribute = 0; iAttribute < m_Instances.numAttributes(); iAttribute++)
      {
        for (int iParent = 0; iParent < m_Instances.numAttributes(); iParent++)
        {
          // se o pai na matriz não for nulo
          if (adjMatrix[m_order[iParent]][m_order[iAttribute]] != 0)
          {
            if (m_ParentSets[iAttribute].contains(iParent))
            {
              nCorrect++;
            }
          }
  ...
  public int missingArcs(int[][] adjMatrix, BayesNet other)
  {
	  try
	  {
		  Sync(other);
          ...
          for (int iAttribute = 0; iAttribute < m_Instances.numAttributes(); iAttribute++)
          {
        	  ...
        	  parentSet = m_ParentSets[iAttribute];
        	  
        	  for (int iParent = 0; iParent < parentSet.getNrOfParents(); iParent++)
        	  {
        		  if (adjMatrix[m_order[parentSet.getParent(iParent)]][m_order[iAttribute]] == 0
        				  && adjMatrix[m_order[iAttribute]][m_order[parentSet.getParent(iParent)]] == 0)
        		  {
        			  nMissing++;
        		  }
        	  }
  ...
  public int extraArcs(BayesNet other) {
	    try {
	      Sync(other);
	      int[] reversed_order = new int[m_Instances.numAttributes()];
	      for (int i = 0; i < m_Instances.numAttributes(); i++) {
	    	  reversed_order[m_order[i]] = i;
	      }
	      int nExtra = 0;
	      for (int iAttribute = 0; iAttribute < m_Instances.numAttributes(); iAttribute++) {
	        for (int iParent = 0; iParent < other.getParentSet(m_order[iAttribute]).getNrOfParents(); iParent++) {
	          int variableOther = m_order[iAttribute];
	          ParentSet parentsOther = other.getParentSet(variableOther);
	          int nParent = parentsOther.getParent(iParent);
	          int parentHere = reversed_order[nParent];
	          if (!m_ParentSets[iAttribute].contains(parentHere) && !m_ParentSets[parentHere].contains(iAttribute)) {
	            nExtra++;
	          }
	        }
	      }
  ...
  public int extraArcs(int[][] adjMatrix, BayesNet other) {
	    try {
	      Sync(other);
          ...
          for (int iAttribute = 0; iAttribute < m_Instances.numAttributes(); iAttribute++)
          {
        	  for (int iParent = 0; iParent < m_Instances.numAttributes(); iParent++)
        	  {
        		  ...
        		  {
        			  if (!m_ParentSets[iAttribute].contains(iParent)
        					  && !m_ParentSets[iParent].contains(iAttribute))
        			  {
        				  nExtra++;
        			  }
        		  }
  ...
  public int reversedArcs(int [][] adjMatrix, BayesNet other)
  {
	  try 
	  {
		  Sync(other);
